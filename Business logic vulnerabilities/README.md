...
Kẻ tấn công lợi dụng lỗi để vượt qua các quy tắc và ràng buộc của hệ thống (ví dụ: hoàn tất giao dịch mà không qua bước thanh toán, sửa đổi các giá trị dữ liệu quan trọng, hoặc gửi dữ liệu vô lý để thao túng máy chủ).

Các lỗi logic rất đa dạng và thường là "độc bản", phụ thuộc hoàn toàn vào chức năng riêng của từng ứng dụng.

Rất khó bị phát hiện bằng các phần mềm quét lỗ hổng tự động.
...
- Gây ảnh hưởng nghiêm trọng đến bảo mật tổng thể. Kẻ tấn công có thể leo thang đặc quyền, vượt qua xác thực để truy cập dữ liệu/chức năng nhạy cảm và mở rộng bề mặt tấn công.
...
VD: Nếu một hàm chuyển tiền chỉ kiểm tra số_tiền_chuyển <= số_dư_hiện_tại. Kẻ tấn công nhập số tiền là -1000$. Hệ thống đánh giá -1000 nhỏ hơn số dư nên duyệt giao dịch. Hậu quả: Kẻ tấn công rút ngược 1000$ từ tài khoản nạn nhân thay vì chuyển đi.
...
Kiểm tra ta thấy khi thay đổi số lượng sản phẩm để thêm vào giỏ hàng thì số lượng thay đổi theo, ta thao túng số lượng sản phẩm về âm để số tiền phải trả nhỏ hơn 0
...
Đăng nhập vào tài khoản wiener, bắt request và gửi đến Repeater, kiểm tra nếu số lượng là 99 ok, 100 thì không ok
...
Tính toán số lượng thêm số lượng sản phẩm. Vì khi thêm nhiều số lượng `Lightweight "l33t" Leather Jacket` thì đến 1 mức độ nào đó sẽ vượt ngưỡng tức là dùng kiểu `int` nên số tiền vượt quá giá trị mã của int sẽ trả về giá trị âm nên cần thêm số lượng sản phẩm để triệt tiêu. Sau đó thực hiện đặt hàng để solve bài lab
...
Sửa lại email sao cho đến @dontwannacry.com là 255 ký tự ta sẽ đăng nhập được vào mail @dontwannacry.com sẽ có panel của admin
...
Vượt qua kiểm tra ban đầu ≠ luôn đáng tin cậy
...
- Parameter có/không có → quyết định code path.
- Xóa parameter → truy cập code path không mong muốn.
...
Gửi request này tới Repeater, thực hiện nếu xóa bớt 1 trường thông tin thì có ok không, ta thấy nếu xóa current-password thì việc đổi mật khẩu vẫn diện ra
...
- Xóa bớt hàng.
- Nếu hệ thống không kiểm tra lại điều kiện → vẫn được hưởng giảm giá.
...
Áp dụng voucher này vào để được giảm giá. Nếu áp dụng cùng 1 voucher 2 lần liên tiếp thì không được, nhưng nếu áp dụng chúng xen kẽ thì vẫn áp dụng được
...
Nếu ứng dụng còn tồn tại decryption oracle (cho phép giải mã dữ liệu do người dùng cung cấp), attacker có thể giải mã dữ liệu để xác định cấu trúc plaintext mà hệ thống mong đợi.
